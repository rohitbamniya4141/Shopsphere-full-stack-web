...
export default function AdminProductsPage() {
  ...
  if (isLoading) {
    return <LoadingSpinner fullScreen text="Loading product inventory..." />;
  }
    ...
    <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 py-10 sm:py-14 animate-fade-in-up">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <div className="flex items-center gap-3 mb-1.5">
            ...
            <h1 className="text-3xl sm:text-4xl font-bold text-dark-900 tracking-tight">Platform Catalog</h1>
          </div>
          <p className="text-dark-400 text-sm ml-5 sm:ml-6">
            All listed products across verified sellers ({products.length} items)
          </p>
        </div>
      ...
      {products.length === 0 ? (
        <EmptyState
          ...
          description="Click below to add the first product to ShopSphere."
                ...
                {products.map((p) => {
                  ...
                  return (
                    <tr key={p._id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="py-4 px-6 text-dark-600 font-medium truncate max-w-[120px]">
                        {p.seller?.shopName || 'Admin / ShopSphere'}
                      </td>
